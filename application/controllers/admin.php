...
class Admin extends CI_Controller {
    ...
    public function cargar_imagen_categoria(){
        if($this->session->userdata('user')){
            ...
            if(($elemento == 'portada' || $elemento == 'encabezado') && $tipo === 'image/jpeg'){
                $ext = '.jpg';
                $ruta='./catalogo/' . $id . '/config/' . $elemento . $ext;
                move_uploaded_file($image,$ruta);
            }else if($elemento == 'icono' && $tipo === 'image/png'){
                $ext = '.png';
                $ruta='./catalogo/' . $id . '/config/' . $elemento . $ext;
                move_uploaded_file($image,$ruta);
            }else{
    ...
    public function nueva_imagen_rotativo(){
        if($this->session->userdata('user')){
            ...
            $full = './catalogo/slider/' . $nombre;
            move_uploaded_file($image,$full);

            $slider600 = './catalogo/slider/scale/600_' . $nombre;
            copy($full, $slider600);
            $this->escalar_imagen($slider600, 600);

            $slider992 = './catalogo/slider/scale/992_' . $nombre;
            copy($full, $slider992);
            $this->escalar_imagen($slider992, 992);

            $slider1200 = './catalogo/slider/scale/1200_' . $nombre;
            copy($full, $slider1200);
            $this->escalar_imagen($slider1200, 1200);
        }else{
            http_response_code(401);
        }
    }
    ...
    public function guardar_producto(){
        if($this->session->userdata('user')){
            ...
            $mas_vendido = $this->input->post('masVendido');
            ...
            $this->load->model('m_admin');
            if(!$nuevo){
                $this->m_admin->actualizar_producto($codigo,$id_categoria,$tipo,$tela,$talla_unica,$tallas,$observaciones,$precio,$descuento_especial,$imgMask,$mas_vendido);
                $this->cambiar_imagenes($codigo,$id_categoria,$eliminar,$crear);
                http_response_code(204);
            }else{
                $p = $this->m_admin->nuevo_producto($codigo,$id_categoria,$tipo,$tela,$talla_unica,$tallas,$observaciones,$precio,$descuento_especial,$imgMask,$mas_vendido);
                if($p){
                    $this->cambiar_imagenes($codigo,$id_categoria,$eliminar,$crear);
                    http_response_code(204);
                }else{
    ...
    public function incrementogroups(){
        if($this->session->userdata('user')){
            $categoryId = $this->input->post('id');
            $incrementPercent = $this->input->post('incremento');
            $this->load->model('m_admin');
            $increment = $this->m_admin->incrementos($categoryId,$incrementPercent);
            $referencesItems = $this->m_site->getReferencesWithCategory($categoryId);
            $response = $this->m_admin->addSyncIncrementos($referencesItems, $categoryId, $incrementPercent);
            echo json_encode(array('id' => $response, 'updated' => $increment['updated'], 'query' => $increment['query'], 'listToUpdate' => ['id' => $response['id'], 'data' => $response['data']]));
        }else{
            http_response_code(401);
        }
    }
