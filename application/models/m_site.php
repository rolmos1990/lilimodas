...
class M_site extends CI_Model{
    ...
    function get_productos($id_categoria,$agotados=false){
        if($agotados){
            $query = $this->db->query("select producto.*,categoria.*,if(producto.descuento_especial > 0,CEIL(producto.precio*((100-producto.descuento_especial)/100)),CEIL(producto.precio*((100-categoria.descuento)/100))) as precio_descuento,CEIL(producto.precio*((100-categoria.descuento_mayor_1)/100)) as precio_descuento_mayor_1,producto.descuento_especial "
                    ...
                    . "where id_categoria=$id_categoria order by producto.orden");
        }else{
            $query = $this->db->query("select producto.*,categoria.*,if(producto.descuento_especial > 0,CEIL(producto.precio*((100-producto.descuento_especial)/100)),CEIL(producto.precio*((100-categoria.descuento)/100))) as precio_descuento,CEIL(producto.precio*((100-categoria.descuento_mayor_1)/100)) as precio_descuento_mayor_1,producto.descuento_especial "
                    . "from producto inner join categoria on producto.id_categoria=categoria.id "
                    . "where id_categoria=$id_categoria and producto.existencia>0 order by producto.orden");
        }
        return $query->result();
    }
    function get_productos_todos(){
            $query = $this->db->query("select codigo "
                    . "from producto "
                    . "order by sin_numeros(producto.codigo),numeros(producto.codigo)");
        return $query->result();
    }
    ...
    function get_premios(){
        $query = $this->db->get_where('premio',array(
            'disponible' => true
        ));
        return $query->result_array();
    }
    
    function updatePrices($porcentaje, $category = false){
        $testmode = '';
        if($category == false){
            $sql="UPDATE producto set precio = round(precio + ((precio * ".intval($porcentaje).") / 100), 0)" . $testmode;
        } else {
             $sql="UPDATE producto set precio = round(precio + ((precio * ".intval($porcentaje).") / 100), 0) where id_categoria = " . $category;
        }
        $this->db->query($sql);
        return $this->db->affected_rows();
    }
    
    function getReferencesWithCategory($category){
    ...
    function getPendingSyncReferencesIncrements(){
        $this->db->where('status',1);
        $this->db->order_by('fecha', 'asc');
        $query = $this->db->get('increments');
        $increments = $query->result_array();
        
        return $increments;
    }
    
    function updatePendingSyncReferencesIncrements($id){
        $this->db->where('id', $id);
        $this->db->update('increments', array('status' => 0));
        return $this->db->affected_rows();
    }
}
