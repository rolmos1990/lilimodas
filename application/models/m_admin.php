...
class M_Admin extends CI_Model{
    ...
    function categoria($id){
        ...
        $this->db->select('codigo, precio, existencia, fecha, imagenes, masVendido');
    ...
    function producto($codigo){
        $this->db->select('codigo,tipo,tela,talla_unica as tallaUnica,tallas,observaciones,precio,imagenes,fecha,id_categoria as idCategoria,descuento_especial as descuentoEspecial, masVendido');
        $this->db->where('codigo',$codigo);
        $query = $this->db->get('producto');
        return $query->row();
    }
    function nuevo_producto($codigo,$id_categoria,$tipo,$tela,$talla_unica,$tallas,$observaciones,$precio,$descuento_especial,$imagenes,$masVendido){
    ...
    function actualizar_producto($codigo,$id_categoria,$tipo,$tela,$talla_unica,$tallas,$observaciones,$precio,$descuento_especial,$imagenes,$masVendido){
        $data = array(
            'codigo' => $codigo,
            'id_categoria' => $id_categoria,
            'tipo' => $tipo,
    ...
    function incrementos($categoryId,$incrementPercent){
        $this->load->model('m_site');

        $sql="UPDATE producto set precio = round(precio + ((precio * ".intval($incrementPercent).") / 100), 0) where id_categoria = " . $categoryId;
             
        $this->db->query($sql);
        return ['updated' => $this->db->affected_rows(), 'query' => $sql];
    }
    
    function addSyncIncrementos($references, $categoryId, $percent){
        ...
        $this->db->insert('increments',$data);
        return ['id' => $this->db->insert_id(), 'data' => $data];
    }
